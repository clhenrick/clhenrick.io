...
- **Internationalization (i18n) & localization (l10n):** what happens to the design when it is translated to different languages? When "right to left" (RTL) such as Arabic and Hebrew are used? When translating labels, instructional text, numbers, dates, etc. from English to other languages we have to be mindful that the space requirements will likely be different and our design should plan for this so that users of non-English languages are still able to use our product. There are JavaScript libraries and browser APIs for handling i18n and l10n that make this a problem that UX prototyping can effectively address.
